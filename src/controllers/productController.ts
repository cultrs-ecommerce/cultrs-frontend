import { db } from "../firebaseConfig"; // Assuming your firebase config is exported as db
import { collection, addDoc, Timestamp } from "firebase/firestore";
import { Product } from "@/types/Product";

// Assuming your Product interface doesn't include the File objects for images
interface ProductDataWithoutImages extends Omit<Product, 'imageUrls' | 'createdAt' | 'updatedAt' | 'status' | 'likesCount' | 'viewsCount'> {}
...
export const createProduct = async (productData: ProductDataWithoutImages, imageFiles: File[]): Promise<void> => {
  try {
